...
Many situations in ecology and evolution, absolute or magnitude of effect size is of most interested rather than effect size estimates with directionality (see Morrissey 2016). However, effect magnitude needs to be modeled correctly to avoid bias. As such, for users interested in effect magnitude we have developed a `foldnorm` function which can apply results from normal meta-analytic models (that assume Gaussain error distributions) to generate corrected effect magnitude and credible intervals using an 'anlayse then transform' approach suggested by Morrissey (2016)

**NOTE**: These are planned functions and have not yet been effectively tested or optimized.
